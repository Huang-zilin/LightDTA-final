...
class RWNet(nn.Module):
    def __init__(self, n_output=1, output_dim=128, num_features_xd=78, num_features_pro=33, num_features_ppi = 64):
        ...
        self.mha1 = MultiheadAttention(output_dim, num_heads)
        self.lin = nn.Linear(64, 128)
        self.relu2 = nn.ReLU()

    def forward(self, mol_data, pro_data, ppi_edge, new_ppi_features):
        ...
        # combination
        xc1 = torch.cat((x, ppi_x), 1)
        xc = self.dropout1(self.relu(self.fc1(xc1)))
...
class TDNet(nn.Module): #Top-Down strategy
    ...
    def forward(self, mol_data, pro_data, ppi_edge, ppi_features, pro_graph):
        ...
        # combination
        xc1 = torch.cat((x, p_x), 1) #在这一步输出会导致重新训练
        # classifier
        xc = self.dropout1(self.relu(self.fc1(xc1)))
        xc = self.dropout1(self.relu(self.fc2(xc)))
        embedding = xc
