...
def test(model, device, loader, ppi_adj, new_ppi_features):
    ...
    with torch.no_grad():
        for data in loader:
            ...
            output = output[0]  # 提取元组中的第一个元素（张量）
...
def main(args):
    ...
    with open('/media/ST-18T/xiaoyu/RWDTA/data/davis/PPI/ppi_data_wv.pkl', 'rb') as file3:
    ...
    test_loader = DataLoader(test_dataset, batch_size=args.batch, shuffle=False, collate_fn=collate)
